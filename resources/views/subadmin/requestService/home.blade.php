...
@section('title') {{__('cp.RequestService_common')}}
@endsection
@section('css')
@endsection
        ...
        <div class="portlet-body">
            <div class="table-toolbar">
                ...
                <div class="box-filter-collapse">
                    <form class="form-horizontal" method="get" action="{{url(getLocal().'/subadmin/serviceRequest')}}">
                        <div class="row">
                            <div class="col-md-4">
                            ...
                            <div class="col-md-4">
                                <div class="row">
                                    <div class="col-md-offset-3 col-md-9">
                                        ...
                                        <a href="{{url(app()->getLocale().'/subadmin/serviceRequest')}}" type="submit"
            ...
            <table class="table table-striped table-bordered table-hover table-checkable order-column" id="toolsTable">
                ...
                <tr>
                    ...
                    <th> {{ucwords(__('cp.username_common'))}}</th>
                    <th> {{ucwords(__('cp.price_common'))}} ({{__('cp.kd_common')}})</th>
                ...
                @forelse($items as $item)
                    ...
                    <tr class="odd gradeX" id="tr-{{$item->id}}">
                        ...
                        <td>
                            @if($item->requestService->service) {{$item->requestService->service->name}} @endif

                        </td>
                        <td>

                            {{$item->requestService->store->name}}

                        </td>
                        <td>

                            {{$item->requestService->user->name}}

                        </td>

                        <td>{{$item->price}}</td>
                        <td>{{$item->requestService->number_page}}</td>
                        <td>
                         @if($item->status == 0 )
                        ...
                        <td>
                            <div class="btn-group btn-action">
                                <a href="{{url(getLocal().'/subadmin/serviceRequest/'.$item->id.'/edit')}}"
                                   class="btn btn-xs blue tooltips" data-container="body" data-placement="top"
                                   data-original-title="{{__('cp.edit_common')}}"><i class="fa fa-edit"></i></a>
                                <a data-placement="top" data-original-title="{{__('cp.delete_common')}} " href="#myModal{{$item->id}}" role="button"  data-toggle="modal" class="btn btn-xs red tooltips">
...
@section('script')
    <script>
         ...
         function delete_adv(id,iss_id, e) {
            ...
            console.log(id);
            console.log(iss_id);
            var url = '{{url(getLocal()."/subadmin/serviceRequest")}}/' + id;
