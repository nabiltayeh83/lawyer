...
@section('title') {{ucwords(__('cp.edit'))}} {{ucwords(__('cp.page'))}}
@endsection
@section('css')
@endsection
@section('content')
                ...
                <div class="portlet-title">
                    <div class="caption">
                        ...
                        <span class="caption-subject font-dark sbold uppercase"
                              style="color: #e02222 !important;">{{__('cp.edit')}}{{__('cp.page')}}</span>
                    </div>
                </div>
                            ...
                            @foreach($locales as $locale)
                                <fieldset>
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-6">
                                            <input type="text" disabled class="form-control" name="title_{{$locale->lang}}" value="{{$item->translate($locale->lang)->title}}" id="order"
                                                   placeholder="{{__('cp.title')}} {{$locale->name}}" {{ old('title_'.$locale->lang) }}>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-6">
                                                <textarea class="ckeditor form-control" name="description_{{$locale->lang}}" id="order"
                                                          placeholder="{{__('cp.description')}} {{$locale->name}}" {{ old('description_'.$locale->lang) }}>{{$item->translate($locale->lang)->description}}</textarea>
                                            <span class="help-inline"> * required </span>
                                        </div>
                                    </div>
                                </fieldset>
                            ...
                            @foreach($locales as $locale)
                                <fieldset>
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-6">
                                            <input type="text" disabled class="form-control" name="key_words_{{$locale->lang}}" value="{{$item->translate($locale->lang)->key_words}}" id="order"
                                                   placeholder="{{__('cp.key_words')}} {{$locale->name}}" {{ old('key_words_'.$locale->lang) }}>
                                            <span class="help-inline"> * required </span>
                                        </div>
                                    </div>
                                </fieldset>
